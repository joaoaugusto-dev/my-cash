...
This project also includes a Vercel serverless handler under `api/[...path].ts`, so you can deploy it with:

```bash
vercel --prod
```

After deployment, use the generated URL with the `/api` suffix in the Flutter app's `API_BASE_URL`.
